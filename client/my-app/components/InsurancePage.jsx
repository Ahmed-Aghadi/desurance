...
import { desuranceAbi, currency } from "../constants"
...
function InsurancePage() {
    ...
    const checkUserRequestStatus = async (requestId) => {
        ...
        console.log("request", request.accepted.toString(), tM)
        if (request.accepted.toString() == tM) {
    ...
    const fetchFromContract = async () => {
        try {
            const contractInstance = new ethers.Contract(
                router.query.insuranceAddress,
                desuranceAbi,
                signer ? signer : ethers.getDefaultProvider(process.env.NEXT_PUBLIC_RPC_URL)
            )
